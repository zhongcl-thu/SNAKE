...
'''implemented by 
https://github.com/autonomousvision/convolutional_occupancy_networks/blob/master/src/encoder/pointnet.py'''
...
class LocalPoolPointnet(nn.Module):
    ...
    Args:
        ...
        unet3d (bool): whether to use 3D U-Net
        unet3d_kwargs (str): 3D U-Net parameters
        plane_resolution (int): defined resolution for plane feature
        grid_resolution (int): defined resolution for grid feature (whole volume)
    ...
    def __init__(self, c_dim=128, dim=3, hidden_dim=128, scatter_type='max', 
                 ...
                 padding=0.0, n_blocks=5, z_max=0.5, z_min=-0.5):
        super().__init__()
        self.c_dim = c_dim
    ...
    def generate_grid_features(self, p, c):
        ...
        fea_grid = c.new_zeros(p.size(0), self.c_dim, self.dim_size)
        c = c.permute(0, 2, 1)
        fea_grid = scatter_mean(c, index, out=fea_grid) # B x C x reso^3
        fea_grid = fea_grid.reshape(p.size(0), self.c_dim, 
                                self.reso_grid[2], self.reso_grid[1], self.reso_grid[0]) # sparce matrix (B x 512 x reso x reso)
    ...
    def pool_local(self, xy, index, c):
        ...
        for key in keys:
            # scatter plane features from points
            if key == 'grid':
                fea = self.scatter(c.permute(0, 2, 1), index[key], dim_size=self.dim_size)
            else:
